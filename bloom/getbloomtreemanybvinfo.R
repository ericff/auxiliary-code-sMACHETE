...
## See also getbvinfo.R for other projects
...
projname <- "ericfg/bloomtreemany"

cancer.type.keys <- c("coad", "sarc", "gbm")

cancer.types <- c("Colon Adenocarcinoma", "Sarcoma","Glioblastoma Multiforme")
...
##
## get sample ids and cancer types from .bv files
for (ii in 1:n.bv){
    if (ii %% 10 == 0){
    ...
    cancer.types.bv[ii] <- thisfileinfo$metadata$disease_type
}


## don't know why this doesn't work, but no time now
## cancer.type.keys.bv.files <- replace(x= cancer.types.bv, list=cancer.types, values=cancer.type.keys)
...
bv.all.filename <- file.path(fileinfodir, paste0("bv.bloomtreemany.files.info.", nicedate, ".csv"))
...
## Now narrow down to particular disease types, do loop
## can read in starting here if you want to save the time
## of running the above loop, e.g. if no new files:
## bv.all.df <- read.table(bv.all.filename, header=TRUE, sep=",", stringsAsFactors = FALSE)



for (tti in 1:n.keys){
    this.bv.filename <- file.path(fileinfodir, paste0("bv.", cancer.type.keys[tti], ".files.info.csv"))
    bv.this.df <- bv.all.df[bv.all.df$key==cancer.type.keys[tti],]
    write.table(bv.this.df, file=this.bv.filename, append = FALSE, col.names = TRUE, row.names=FALSE, sep =",")
}
